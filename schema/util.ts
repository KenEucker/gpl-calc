...
import type { Card, Player } from "src/lib"
...
export const slugifyCard = async (doc: any, client: SanityClient) => {
    ...
    let winnerName = ''
    await client.fetch(query, {playerRef: doc.winner._ref ?? doc.winner._id}).then((player: Player) => {
    /// Set the referenceName to the category.name field
    winnerName = !!player ? player.name : 'winner'
    })
    let loserName = ''
    await client.fetch(query, {playerRef: doc.loser._ref ?? doc.loser.id}).then((player: Player) => {
    /// Set the referenceName to the category.name field
